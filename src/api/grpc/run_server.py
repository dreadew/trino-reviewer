...
class GRPCApplication:
    """gRPC приложение для анализа схемы БД."""
    ...
    def initialize(self) -> bool:
        ...
        """
        try:
            self.logger.info(f"Инициализация gRPC приложения {config.APP_NAME}")
    ...
    def start(self) -> None:
        ...
        if not self.initialize():
            self.logger.error("Не удалось инициализировать gRPC приложение")
            sys.exit(1)
        ...
        try:
            self.logger.info(f"Запуск gRPC сервера {config.APP_NAME}...")
            self.grpc_server.start()
        except Exception as e:
            self.logger.error(f"Ошибка запуска gRPC приложения: {e}")
            sys.exit(1)
    ...
    def stop(self) -> None:
        ...
        """
        self.logger.info("Остановка gRPC приложения...")
        if self.grpc_server:
            self.grpc_server.stop()
        self.logger.info("gRPC приложение остановлено")
    ...
    def _signal_handler(self, signum, frame) -> None:
        ...
        """
        self.logger.info(f"Получен сигнал {signum}, остановка gRPC приложения...")
        self.stop()
        sys.exit(0)
